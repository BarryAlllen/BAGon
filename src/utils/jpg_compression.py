...
def dct_coeff(n, k, N):
    # This function returns a value representing a coefficient in the Discrete Cosine Transform (DCT).
    return np.cos(np.pi / N * (n + 1. / 2.) * k)


def idct_coeff(n, k, N):
    # This function returns a value representing a coefficient in the Inverse Discrete Cosine Transform (IDCT).
...
class JpegCompression(nn.Module):
    ...
    def forward(self, img):
        # 1. Preprocess the image (pad the length and width to multiples of 8)
        noised_image = img
...
#
# img = Image.open("/workspace/code/watermark/bagon/data/result/benchmark/image/000000470121.jpg")
#
# noised_image = transforms.ToTensor()(img)
#
# noised_image = noised_image.unsqueeze(0).to(device)
# print("Noised Image Shape:", noised_image.shape)
#
# jpg_layer = JpegCompression(device, yuv_keep_weights=(25, 9, 9))
# jpg_img = jpg_layer(noised_image)
# print("JPG Image Shape:", jpg_img.shape)
#
...
#
# save_tensor_as_image(noised_image, "/workspace/code/watermark/bagon/data/result/benchmark/image/noised_image.jpg")
#
# save_tensor_as_image(jpg_img, "/workspace/code/watermark/bagon/data/result/benchmark/image/jpg_image.jpg")
