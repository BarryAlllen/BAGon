...
class EncoderV0(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.conv_block_1_1 = DoubleConvBatchNormReluBlock(in_channels=in_channels, out_channels=16)
        self.conv_block_2_1 = DoubleConvBatchNormReluBlock(in_channels=16, out_channels=32)
        self.conv_block_3_1 = DoubleConvBatchNormReluBlock(in_channels=32, out_channels=64)

        self.conv_block_1_2 = DoubleConvBatchNormReluBlock(in_channels=16 * 2 + 64, out_channels=16)
        self.conv_block_2_2 = DoubleConvBatchNormReluBlock(in_channels=32 * 2 + 64, out_channels=32)
        self.conv_block_3_2 = DoubleConvBatchNormReluBlock(in_channels=64 * 3, out_channels=64)
    ...
    def forward(self, x, message):
        ...
        message_1 = self.message_process(message=message, height=x11.shape[2], width=x11.shape[3])
        x11 = torch.cat(tensors=(x1, x11, message_1), dim=1)
        x11 = self.conv_block_1_2(x11)

        out = self.channel_adjust(x11)
        return out


class EncoderV2_1(nn.Module):
    """
    3 attention base v0
    """
    def __init__(self, in_channels=3, out_channels=3):
...
class EncoderV1_2(nn.Module):
    """
    v1_1 the same, but the params name
    """
    def __init__(self, in_channels=3, out_channels=3):
        super().__init__()
        self.name = f"Model: {self.__class__.__name__}"
...
class EncoderV3(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.conv_block_2_1 = DoubleConvBatchNormReluBlock(in_channels=16, out_channels=32)
        self.conv_block_3_1 = DoubleConvBatchNormReluBlock(in_channels=32, out_channels=64)
        self.conv_block_5_1 = ConvBatchNormReluBlock(in_channels=64, out_channels=128)
        self.conv_block_5_2 = ConvBatchNormReluBlock(in_channels=128, out_channels=64)
    ...
    def forward(self, x, message):
        ...
        x4 = self.down_sample_2x2(x3)
        x5 = self.down_sample_4x4(x4)
        x5 = self.conv_block_5_1(x5)

        x55 = x5.repeat(1, 1, 4, 4)
        x55 = self.attention_5(x55)
        x44 = self.conv_block_5_2(x55)
        message_4 = self.message_process(message=message, is_interpolate=False)
        x44 = torch.cat(tensors=(x4, x44, message_4), dim=1)
